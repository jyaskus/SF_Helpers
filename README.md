...
2. **Companion Defaults Wizard** – Adds a smart sync button on companions that set the creature companion level-based stats from companion pet using system introduced in AA2 (many values of which, currently can be set by the player), also computes the Cost to upgrade a companion to the next level.
